    ...
    public class PhysBoneMaker : MonoBehaviour
    {
        ...
        public void MakeIntoPhys()
        {
            ...
            for (int i = 0; i < bones.Count - 1; i++)
            {
                ...
                var normalSmoother = newSmooth.GetComponent(SmoothFollowerT);
                Set_targetTransform(normalSmoother, physBalls[i].transform);

                foreach (var smth in newSmooth.GetComponentsInChildren(SmoothFollowerT, true))
                {
                    Set_TranslationSmoothTime(smth, .5f / VisualSmoothing);
                    Set_RotationalSmoothTime(smth, .5f / VisualSmoothing);
                }
